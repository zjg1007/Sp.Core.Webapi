...
namespace Blog.Core.Repository
{
    public class DbContext
    {
        ...
        private DbContext(bool blnIsAutoCloseConnection = true)
        {
            ...
            {
                ConnectionString = _connectionString,//数据库连接字符串
                DbType = _dbType,//设置数据库类型
                IsAutoCloseConnection = blnIsAutoCloseConnection,//自动释放数据务，如果存在事务，在事务结束后释放
                IsShardSameThread = false,
                InitKeyType = InitKeyType.Attribute, //从实体特性中读取主键自增列信息---   没有的话不会自动创建数据库
