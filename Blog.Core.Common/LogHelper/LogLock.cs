    ...
    public class LogLock
    {
        ...
        public static void OutSql2Log(string filename, string[] dataParas)
        {
            try
            {
                ...
                {
                    Console.WriteLine("NoFile:"+path+"没有找到文件，开始创建文件中-------");
                ...
                File.AppendAllText(logFilePath, logContent, System.Text.Encoding.UTF8);
                WritedCount++;
            }
            catch (Exception e)
            {
        ...
        public static List<LogInfo> GetLogData()
        {
            ...
            try
            {
                var strMsg = ReadLog(Path.Combine(contentRoot, "Log", "AOPLog.log"), Encoding.UTF8);
            ...
            try
            {
                var strMsg = ReadLog(Path.Combine(contentRoot, "Log", $"GlobalExcepLogs_{DateTime.Now.ToString("yyyMMdd")}.log"), Encoding.UTF8);
                if (!string.IsNullOrEmpty(strMsg))
                {
            ...
            try
            {
                var strMsg = ReadLog(Path.Combine(contentRoot, "Log", "SqlLog.log"), Encoding.UTF8);
                if (!string.IsNullOrEmpty(strMsg))
                {
            ...
            try
            {
                var strMsg = ReadLog(Path.Combine(contentRoot, "Log", "RequestResponseLog.log"), Encoding.UTF8);
                if (!string.IsNullOrEmpty(strMsg))
                { 
